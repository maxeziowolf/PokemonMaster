...
final class ServiceCoordinatorTest: XCTestCase {
    ...
    func testInvalidURL() {
        ...
        ServiceCoordinator.sendRequest(url: invalidURL) { (response: ServiceStatus<MockModel>) in

            switch response {

            case .success(data: let data):
                XCTFail("Debió votar un error de URL Invalida y regreso una de \(data)")
            case .failed(error: let error):
                XCTAssertEqual(error, ServiceErrorMessage.urlInvalid, "Debió votar un error de URL Invalida ")
    ...
    func testSendRequestWithHeaderFields() {
        ...
        ServiceCoordinator.sendRequest(url: url, headerFields: headers) { (response: ServiceStatus<[MockModel]>) in
    ...
    func testSendRequestWithBody() {
        ...
        ServiceCoordinator.sendRequest(url: url, httpMethod: .post, body: body) { (response: ServiceStatus<[MockModel]>) in
    ...
    func testResponseWithError() {
        ...
        ServiceCoordinator.sendRequest(url: url) { (response: ServiceStatus<[MockModel]>) in
            ...
            case .success(data: let data):
                XCTFail("Debió dar un error personalizado, pero regreso la siguiente informacion\(data)")
            case .failed(error: let error):
                XCTFail("Debió dar un error personalizado\(error)")
            case .unowned(error: let error):
                XCTAssertEqual(error, error.description, "Los errores no conciden con el probocado")
            }
            errorExpected.fulfill()
        }
    ...
    func testResponseWithCode100To200() {
        ...
        ServiceCoordinator.sendRequest(url: url) { (response: ServiceStatus<[MockModel]>) in
            ...
            case .unowned(error: let error):
                XCTAssertEqual(error, "Se respondio solo con informacion", "El error no es el correcto")
            }
            errorExpected.fulfill()
        }
